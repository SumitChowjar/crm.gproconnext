...
//to call drag-drop
$(document.body).on("click", ".utilsLink", function(){

    var course_id = $('div.favorite').attr('record_id');

    //remove unnecessary attributes as we are craeting our custom subpanel
    $("table.subpanel-table").find("button[name='listViewStartButton']").parent('td').remove();

    var subpanel_exist = $("table.subpanel-table").children('tbody').find("tr[custom='y']").length;

    if(subpanel_exist == 0) {
        $("table.subpanel-table").children('tbody').children('tr.oddListRowS1, tr.evenListRowS1').remove();
        $.ajax({
            url: 'get_course_resources.php',
            method: 'post',
            data: {
                id : course_id
            },
            success:function (response) {
                ...
                $("table.subpanel-table").children('tbody').append(response);
            },
    ...
    $("table.subpanel-table").children('tbody').children('tr').each(function (index, value) {
        $(value).addClass('nodrop nodrag');
        return (index !== 2);
    });

    $("table.subpanel-table").tableDnD({
            //onDragClass: "myDragClass",
            onDrop: function(table, row) {
                var rows = table.tBodies[0].rows;
                $("table.subpanel-table").find("tr.oddListRowS1, tr.evenListRowS1").removeClass("evenListRowS1 oddListRowS1").addClass('dragging');
                $("table.subpanel-table").find("tr.dragging:even").addClass("oddListRowS1");
                $("table.subpanel-table").find("tr.dragging:odd").addClass("evenListRowS1");
                $("table.subpanel-table").find("tr.dragging").removeClass("dragging");
