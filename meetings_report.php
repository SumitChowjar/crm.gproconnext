     ...
     $end_date = date("Y-m-d", strtotime($_POST['end_date']));
       ...
       global $db;
            ...
            $query= "SELECT DISTINCT 
                    (SELECT COUNT(*) FROM meetings WHERE status='Planned' AND deleted = 0) as Planned, 
                    (SELECT COUNT(*) FROM meetings WHERE status='Held'AND deleted = 0) as Held, 
                    (SELECT COUNT(*) FROM meetings WHERE status='Not Held' AND deleted = 0) as Not_Held, 
                    (SELECT COUNT(*) FROM meetings WHERE deleted = 0) as TotalCount FROM meetings  WHERE DATE(meetings.date_entered) >= '".$start_date."' AND DATE(meetings.date_entered)<= '".$end_date."' AND meetings.deleted = 0";
         ...
         if($result->num_rows != 0)
         {
          ...
          $str.='<a id="exp" href="javascript:void(0)" class="expandLink" onclick="javascript:myLinkButtonClick();"><img border="0" id="detailpanel_report_img_show" src="themes/Suite7/images/advanced_search.gif?v=N3y-Ekbw475Psgre05yy4Q">';
           ...
           $str.='<th scope="col">';
           $str.='<div style="white-space: normal;" width="100%" align="letf">';
           $str.='<span style="cursor:pointer">Total Meetings</span>';
           $str.='<img src="custom/js/if_sort.png" style="height:12px;">';
           $str.='</div>';
           $str.='</th>';
           $str.='</thead>';
