...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.call_medium_c.name}" 
id="{$fields.call_medium_c.name}" 
title=''       
>
{if isset($fields.call_medium_c.value) && $fields.call_medium_c.value != ''}
{html_options options=$fields.call_medium_c.options selected=$fields.call_medium_c.value}
{else}
{html_options options=$fields.call_medium_c.options selected=$fields.call_medium_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.call_medium_c.options }
{capture name="field_val"}{$fields.call_medium_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.call_medium_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.call_medium_c.name}" 
id="{$fields.call_medium_c.name}" 
title=''          
>
{if isset($fields.call_medium_c.value) && $fields.call_medium_c.value != ''}
...
<input
id="{$fields.call_medium_c.name}-input"
name="{$fields.call_medium_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.call_medium_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.call_medium_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.call_medium_c.name}-input', '{$fields.call_medium_c.name}');sync_{$fields.call_medium_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.call_medium_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.call_medium_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.call_medium_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.call_medium_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.call_medium_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.call_medium_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.call_medium_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{if $fields.status.value != "Held"}<input title="{$APP.LBL_CLOSE_AND_CREATE_BUTTON_TITLE}" id="CLOSE_CREATE_FOOTER" accessKey="{$APP.LBL_CLOSE_AND_CREATE_BUTTON_KEY}" class="button" onclick="SUGAR.calls.fill_invitees(); document.EditView.status.value='Held'; document.EditView.action.value='Save'; document.EditView.return_module.value='Calls'; document.EditView.isDuplicate.value=true; document.EditView.isSaveAndNew.value=true; document.EditView.return_action.value='EditView'; document.EditView.return_id.value='{$fields.id.value}'; formSubmitCheck();" type="button" name="button" value="{$APP.LBL_CLOSE_AND_CREATE_BUTTON_LABEL}"/>{/if}
...
addToValidate('EditView', 'call_medium_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CALL_MEDIUM' module='Calls' for_js=true}{literal}' );
